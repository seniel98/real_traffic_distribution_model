...
def write_simulation_files(db, way_id_list=None, name="default", sim_type="Normal", vehicle_type_dict={}):
    ...
    if way_id_list is not None:
        ...
        write_rou_file(cursor, name, sim_type, "agg", all_vehicles, is_agg_file=True)
        write_additional_file(name, sim_type, vehicle_type_dict)
...
def write_additional_file(name, sim_type, vehicle_type_dict):
    """
    ...
    Args:
      ...
      vehicle_type_dict: A dictionary of vehicle types and their percentages.
    """
...
def write_rou_file(cursor, name, sim_type, vehicle_type, vehicles, is_agg_file=False):
    """
    ...
    with open(file_name, 'w+') as route_file:
        ...
        if not is_agg_file:
            ...
            for element in routes_list:
                ...
                for key, value in char_to_replace.items():
                    edges = edges.replace(key, value)
                route_file.write('\t<route id="%s" edges="%s"/>\n' % (element[0], edges))

            # Write aggregated vehicles
            write_vehicles(route_file, vehicles)
